L’architecture physique du système repose sur plusieurs composants matériels interconnectés via un réseau sécurisé. Les moniteurs Mindray et les caméras OCR collectent les données en temps réel, qui sont ensuite transmises au système central via le réseau de surveillance interne. Les serveurs, hébergés localement ou dans le cloud, stockent et traitent les données, tandis que les appareils mobiles permettent au personnel médical de recevoir des notifications et d’accéder aux données des patients.

\begin{itemize}
    \item \textbf{Moniteurs Mindray} : Collectent les données des patients en temps réel (fréquence cardiaque, pression artérielle, saturation en oxygène, etc.) et les transmettent via des API ou des protocoles HL7.
    \item \textbf{Caméras OCR} : Capturent les écrans des machines anciennes et extraient les données via des techniques de reconnaissance optique de caractères (OCR).
