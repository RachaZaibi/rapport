...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Base de données principale} :
            ...
            \end{itemize}
        % \item \textbf{Base de données pour le cache} :
        %     \begin{itemize}
        %         \item Redis : Version 6.x ou supérieure.
        %         \item Rôle : Gestion du cache pour améliorer les performances du système, notamment pour les requêtes fréquentes.
        %         \item Configuration : Stockage en mémoire pour un accès ultra-rapide aux données.
        %     \end{itemize}
    \end{itemize}

    \item \textbf{Protocoles de communication} :
    \begin{itemize}
        \item \textbf{HL7 (Health Level Seven)} :
            \begin{itemize}
                \item Version : HL7 FHIR (Fast Healthcare Interoperability Resources) R4.
                \item Rôle : Standard pour l'échange de données médicales entre les moniteurs Mindray et le système central.
                \item Fonctionnalités : Support des messages en temps réel (ex. résultats de tests, alertes).
            \end{itemize}
        \item \textbf{API REST} :
            \begin{itemize}
