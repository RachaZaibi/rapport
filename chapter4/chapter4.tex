% \chapter{Planification et préparation du projet Parc Informatique FMM}
% \label{chap:sprint0}
\glennchapter{CHAPITRE 4}{Planification et préparation  du projet Parc Informatique FMM}
\vspace{1cm}
\markboth{Chapitre 4 : Planification et préparation du projet Parc Informatique FMM}{}
\label{chap:sprint0}
\addcontentsline{toc}{chapter}{Chapitre 4 : Planification et préparation du projet Parc Informatique FMM}   
\setcounter{chapter}{4}
\setcounter{section}{0}


\localtableofcontents
\newpage
\pagebreak
\section*{Introduction}
\addcontentsline{toc}{section}{Introduction }
\label{sec:intro_sprint0}

Le Sprint 0 constitue la phase initiale du projet Parc Informatique FMM, qui vise à développer un système de gestion des équipements informatiques, des incidents et des maintenances au sein du SmartLab de la Faculté de Médecine de Monastir. Cette étape fondamentale permet d’identifier les acteurs clés, de recueillir les besoins fonctionnels et non fonctionnels, et de définir la méthodologie Scrum qui guidera les phases ultérieures du développement. L’objectif est d’assurer un alignement avec les ambitions du projet, à savoir améliorer la gestion des actifs informatiques grâce à une collecte automatisée des données, une gestion des incidents en temps réel et des rapports détaillés, comme décrit dans les chapitres d’introduction générale et d’étude architecturale.

\section{Pilotage du projet avec Scrum}
\label{sec:gestion_scrum}

Le projet adopte la méthodologie Scrum pour assurer un développement itératif et une collaboration étroite avec les parties prenantes, comme justifié dans le chapitre consacré à la méthodologie. Cette section détaille les rôles Scrum, l’organisation du backlog et la planification des sprints.

\subsection{Acteurs Scrum et missions}
\label{subsec:roles_scrum}

Les rôles Scrum et leurs responsabilités sont définis dans un tableau spécifique, garantissant une approche collaborative alignée sur les principes décrits précédemment. Le Product Owner est chargé de définir et de prioriser les fonctionnalités du produit, tout en validant les livrables pour s’assurer qu’ils répondent aux besoins du projet. L’équipe de développement conçoit, développe et teste les fonctionnalités, en veillant à la qualité du code produit. Le Scrum Master facilite les événements Scrum, élimine les obstacles et veille à ce que l’équipe adhère aux pratiques de la méthodologie.

\begin{table}[H]
\centering
\caption{Acteurs du projet Scrum}
\label{tab:roles_scrum}
\begin{tabular}{|p{3cm}|p{7cm}|p{3cm}|}
\hline
\textbf{Rôle} & \textbf{Mission} & \textbf{Acteur} \\
\hline
Product Owner & Définir et prioriser les fonctionnalités du produit (Product Backlog), valider les livrables. & Pr Ammeri Charfeddine \\
\hline
Équipe de Développement & Concevoir, développer et tester les fonctionnalités, assurer la qualité du code. & Hela Boussaid \\
\hline
Scrum Master & Faciliter les événements Scrum, supprimer les obstacles, assurer l'adhésion à Scrum. & Racha Zaibi \\
\hline
\end{tabular}
\end{table}

\subsection{Organisation du Backlog}
\label{subsec:backlog}

Le Product Backlog est géré à l’aide d’un outil de gestion de projet et organisé selon la méthode de priorisation MoSCoW, afin de refléter les objectifs stratégiques du projet. Chaque fonctionnalité est formulée sous forme de User Story pour favoriser un développement centré sur l’utilisateur. Les tâches prioritaires couvrent la gestion des équipements informatiques (ajout, modification et suppression), la gestion des incidents (création de tickets et affectation aux techniciens), ainsi que la gestion des maintenances préventives et correctives. Les interfaces web permettent la visualisation des équipements et la gestion des tickets, tandis que l’authentification et la gestion des utilisateurs garantissent un accès protégé basé sur les rôles (administrateur, technicien, utilisateur). Le stockage des données et des historiques dans une base de données relationnelle constitue également une priorité, au même titre que la génération de rapports sur les équipements et les incidents. D’autres tâches, importantes mais non critiques pour la première version, incluent les notifications push pour les nouveaux tickets, la gestion des licences logicielles et l’optimisation des performances pour un grand nombre d’équipements. Des fonctionnalités optionnelles, comme l’intégration avec des outils externes ou le support multilingue, pourront être ajoutées si les ressources le permettent. Enfin, certaines tâches, telles que l’analyse prédictive des pannes par intelligence artificielle, demeurent hors du périmètre actuel.

\begin{table}[H]
\centering
\caption{Backlog du Projet avec Priorités MoSCoW}
\label{tab:backlog}
\begin{tabular}{|p{1.5cm}|p{8cm}|p{1.5cm}|}
\hline
\textbf{ID} & \textbf{Tâche} & \textbf{Priorité} \\
\hline
M-01 & Gestion des équipements : ajout, modification, suppression et visualisation. & M \\
\hline
M-02 & Gestion des incidents : création de tickets, assignation et résolution. & M \\
\hline
M-03 & Gestion des maintenances : préventives et correctives. & M \\
\hline
M-04 & Interface web pour administrateurs et techniciens. & M \\
\hline
M-05 & Authentification et gestion des utilisateurs (rôles : admin, technicien, utilisateur). & M \\
\hline
M-06 & Stockage et historique des données dans une base de données MySQL. & M \\
\hline
M-07 & Génération de rapports sur les équipements et incidents. & M \\
\hline
S-01 & Notifications push pour nouveaux tickets via Firebase FCM. & S \\
\hline
S-02 & Gestion des licences logicielles associées aux équipements. & S \\
\hline
S-03 & Optimisation des performances pour gestion de nombreux équipements. & S \\
\hline
C-02 & Support multilingue pour l'interface. & C \\
\hline
W-01 & Analyse prédictive des pannes avec IA. & W \\
\hline
\end{tabular}
\end{table}

\subsection{Planification des sprints}
\label{subsec:planification_sprints}

Le projet est structuré en trois sprints de trois semaines chacun. Le tableau ci-dessous présente une synthèse des objectifs, livrables, tâches, critères d’acceptation et risques associés à chaque sprint.

\begin{longtable}{|p{3cm}|p{4cm}|p{4cm}|p{4cm}|}
\caption{Planification des Sprints du Projet Parc Informatique FMM}\label{tab:sprints} \\
\hline
\textbf{Sprint} & \textbf{Objectifs et livrables} & \textbf{Tâches principales} & \textbf{Critères d’acceptation}  \\
\hline
\endfirsthead
\hline
\textbf{Sprint} & \textbf{Objectifs et livrables} & \textbf{Tâches principales} & \textbf{Critères d’acceptation}  \\
\hline
\endhead
% Sprint 1
\textbf{Sprint 1 \newline Mise en place de l'architecture et gestion des équipements} &
\begin{itemize}[leftmargin=4pt]
    \item Backend API REST avec Express.js.
    \item Base de données MySQL configurée.
    \item Module de gestion des équipements fonctionnel.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Configuration du serveur backend.
    \item Création des schémas de base de données.
    \item Développement des endpoints pour CRUD des équipements.
    \item Validation fonctionnelle des fonctionnalités.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Équipements ajoutés, modifiés et supprimés via API.
    \item Données persistées correctement en base.
    \item Interface web basique pour visualisation.
\end{itemize}  \\
\hline
% Sprint 2
\textbf{Sprint 2 \newline Gestion des incidents et maintenances} &
\begin{itemize}[leftmargin=4pt]
    \item Système de ticketing opérationnel.
    \item Module de maintenances implémenté.
    \item Notifications push intégrées.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Développement des endpoints pour tickets et maintenances.
    \item Intégration de Firebase FCM pour notifications.
    \item Gestion de l'état des tickets et affectation.
    \item Validation fonctionnelle par tests manuels.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Tickets créés et assignés aux techniciens.
    \item Maintenances générées lors de la clôture des interventions.
    \item Notifications envoyées en temps réel.
\end{itemize} \\
\hline
% Sprint 3
\textbf{Sprint 3 \newline Finalisation et rapports} &
\begin{itemize}[leftmargin=4pt]
    \item Interface web fonctionnelle avec React.
    \item Système de rapports fonctionnel.
    \item Authentification et autorisation protégées.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Développement de l'interface web.
    \item Implémentation de l'authentification JWT.
    \item Génération de rapports PDF/JSON.
    \item Validation de la sécurité et des performances.
\end{itemize} &
\begin{itemize}[leftmargin=4pt]
    \item Interface utilisateur intuitive et responsive.
    \item Rapports générés sur demande.
    \item Accès protégé selon les rôles.
\end{itemize} \\
\hline
\end{longtable}

\section{Environnement de travail}
\label{sec:environnement}

Ce projet est développé sur une configuration locale Windows avec une architecture applicative fondée sur un backend Node.js/Express.js et un frontend React/Vite. Les sections suivantes décrivent l'environnement matériel, l'environnement de développement et l'environnement logiciel utilisés par l'équipe.

\subsection{Environnement matériel}
\label{subsec:materiel}

L'environnement matériel utilisé pour le développement du projet repose sur une machine de travail performante, permettant de garantir de bonnes performances lors de l’exécution des outils de développement, des tests et du déploiement local des applications.

Le poste de développement est un ordinateur portable équipé d’un processeur Intel Core i7 de 10\textsuperscript{e} génération, de 16 Go de mémoire RAM et d’un stockage SSD de 512 Go. Cette configuration assure une exécution fluide des environnements backend et frontend ainsi qu’une bonne réactivité lors de la compilation et des tests.

Le système d’exploitation utilisé est Windows, choisi pour sa stabilité et sa compatibilité avec les technologies employées dans le projet. La base de données MySQL est installée en local afin d’assurer une gestion efficace, rapide et protégée des données du système. Enfin, l’architecture réseau repose sur un environnement local (\textit{localhost}), permettant de réaliser les tests et le développement sans dépendre d’une infrastructure externe.

\vspace{0.5cm}

\noindent
\textbf{Système d’exploitation}

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/windows_logo.png}
\end{center}

Windows est utilisé comme système principal de développement. Il offre une compatibilité avec les outils essentiels tels que Node.js, les environnements de développement intégrés (IDE) et les outils de gestion de bases de données.

\vspace{0.3cm}

\noindent
\textbf{Base de données}

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/mysql_logo.png}
\end{center}

MySQL est utilisé comme système de gestion de base de données relationnelle. Il est installé en local et fonctionne sur le port 3306, assurant la persistance et l’intégrité des données du système.

\vspace{0.3cm}

\noindent
\textbf{Réseau}

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/network_logo.png}
\end{center}

L’environnement réseau repose sur une configuration locale (\textit{localhost}), utilisée pour le développement et les tests, garantissant un déploiement rapide et indépendant de toute infrastructure externe.

\subsection{Environnement de développement}
\label{subsec:dev_environnement}

L’environnement de développement regroupe l’ensemble des outils utilisés pour la conception, l’implémentation et la documentation du projet. Il permet d’assurer un cycle de développement fonctionnel, allant de l’écriture du code jusqu’au test et à la génération de la documentation technique.

L’éditeur principal utilisé est Visual Studio Code, choisi pour sa légèreté, sa richesse en extensions et son support adapté des technologies web modernes.

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/vscode_logo.jpg}
\end{center}

% L'environnement repose sur Node.js accompagné de npm, qui constitue le gestionnaire de paquets permettant l’installation et la gestion des dépendances du projet.
% speak about ern as mysql express react node 

% \begin{center}
% \includegraphics[height=2.5cm]{chapter4/images/javascript_logo.png}
% \end{center}

L'environnement de développement du projet repose sur une architecture JavaScript Full Stack illustrée par la figure \ref{fig:architecture_technique}. Cette architecture combine plusieurs technologies complémentaires permettant de couvrir l'ensemble des besoins de l'application.

Du côté serveur, \textbf{Node.js} est utilisé comme environnement d'exécution JavaScript, tandis que \textbf{Express.js} fournit les mécanismes nécessaires à la conception des services Web REST assurant la communication entre les différentes couches du système. La couche de présentation est développée à l'aide de \textbf{React.js}, une bibliothèque JavaScript moderne facilitant la création d'interfaces utilisateur interactives, réactives et modulaires.

Pour la persistance des données, le projet s'appuie sur le système de gestion de bases de données relationnelles \textbf{MySQL}. L'accès aux données est assuré par \textbf{Sequelize ORM}, qui permet de manipuler les entités métier sous forme d'objets tout en simplifiant les opérations de gestion de la base de données. Cette approche améliore la maintenabilité du code et réduit la complexité liée à l'écriture des requêtes SQL.

L'adoption de cette architecture garantit une séparation claire des responsabilités entre les différentes couches du système, favorise la réutilisabilité du code et facilite l'évolution future de l'application.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{chapter4/images/architecture_technique.jpg}
\caption{Architecture technologique }
\label{fig:architecture_technique}
\end{figure}

Afin d'améliorer la productivité de l'équipe de développement et de garantir la qualité du logiciel réalisé, plusieurs outils complémentaires ont été intégrés à l'environnement de travail :

\begin{itemize}
\item \textbf{Nodemon} : outil permettant le redémarrage automatique du serveur backend à chaque modification du code source, ce qui accélère les cycles de développement et de test.

\item \textbf{Vite} : environnement moderne de développement frontend offrant un temps de démarrage très rapide ainsi qu'un mécanisme de \textit{Hot Module Replacement (HMR)} permettant d'appliquer instantanément les modifications effectuées dans l'interface utilisateur.

\item \textbf{ESLint} : outil d'analyse statique du code JavaScript utilisé pour détecter les erreurs potentielles, appliquer les conventions de codage et maintenir une qualité de code homogène tout au long du projet.

\item \textbf{Git} : système de gestion de versions distribué utilisé pour le suivi des modifications du code source, la gestion des différentes versions du projet ainsi que la collaboration entre les membres de l'équipe.

\item \textbf{Swagger UI} : interface de documentation interactive permettant de visualiser, tester et valider les différents endpoints exposés par l'API REST développée dans le cadre du projet.

\item \textbf{Visual Studio Code} : environnement de développement intégré utilisé pour l'implémentation du frontend, du backend et la gestion des différents fichiers constituant l'application.

\item \textbf{Postman} : outil de test et de validation des services Web REST permettant de vérifier le bon fonctionnement des requêtes HTTP et des réponses retournées par l'API.
```

\end{itemize}


\subsection{Environnement logiciel}
\label{subsec:logiciel}

L’environnement logiciel du projet repose sur une architecture JavaScript fonctionnelle (Full Stack), séparée en backend, frontend et base de données. Cette organisation permet une meilleure modularité, une maintenance simplifiée et une évolutivité du système.

% =========================================================
\subsubsection{Backend}

Le backend est développé sous Node.js et repose sur Express.js pour la création de l’API REST. Il est responsable de la logique métier, de la gestion des utilisateurs, des équipements, des tickets ainsi que de la communication avec la base de données.

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/javascript_logo.png}
\end{center}

Les principales bibliothèques utilisées sont :

\begin{itemize}
    \item \textbf{Sequelize} : ORM facilitant la communication avec MySQL et la gestion des relations.
    \item \textbf{MySQL2} : pilote de connexion à la base de données.
    \item \textbf{JWT (JSON Web Token)} : gestion de l’authentification et sécurisation des API.
    \item \textbf{bcryptjs} : hachage protégé des mots de passe.
    \item \textbf{Firebase Admin} : envoi de notifications push via FCM.
    \item \textbf{Multer} : gestion des uploads de fichiers (images de tickets).
    \item \textbf{PDFKit} : génération de rapports PDF.
    \item \textbf{ExcelJS} : export des données en format Excel.
    \item \textbf{Nodemailer} : envoi d’e-mails transactionnels.
    \item \textbf{Swagger (JSDoc/UI)} : documentation interactive de l’API REST.
    \item \textbf{Morgan} : journalisation des requêtes HTTP.
    \item \textbf{dotenv} : gestion des variables d’environnement.
\end{itemize}

% =========================================================
\subsubsection{Frontend}

Le frontend est développé avec React et Vite, permettant la création d’une interface utilisateur dynamique et réactive adaptée aux besoins du système.

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/react_logo.jpg}
\end{center}

Les principales technologies utilisées sont :

\begin{itemize}
    \item \textbf{React Router DOM} : gestion de la navigation dans l’application SPA.
    \item \textbf{Material UI (MUI)} : bibliothèque de composants UI modernes.
    \item \textbf{MUI DataGrid} : gestion des tableaux de données adaptés.
    \item \textbf{Axios} : communication avec l’API backend.
    \item \textbf{TanStack Query} : gestion du cache et des requêtes serveur.
    \item \textbf{Firebase} : notifications push côté client.
    \item \textbf{Recharts} : visualisation des données sous forme de graphiques.
    \item \textbf{html5-qrcode / qrcode.react} : génération et lecture de QR codes.
    \item \textbf{i18next} : internationalisation (FR/EN).
    \item \textbf{react-hot-toast} : notifications utilisateur.
    \item \textbf{date-fns} : manipulation des dates.
    \item \textbf{lucide-react} : icônes modernes.
\end{itemize}



% =========================================================
\subsubsection{Base de données}

La base de données utilisée est MySQL, qui assure la persistance et l’intégrité des données du système.

\begin{center}
\includegraphics[height=2.5cm]{chapter4/images/mysql_logo.png}
\end{center}

Elle est caractérisée par :

\begin{itemize}
    \item Nom de la base : \textbf{gmao\_parc}
    \item Port : \textbf{3306}
    \item ORM utilisé : \textbf{Sequelize} (migrations et relations)
\end{itemize}

\section*{Conclusion}
\addcontentsline{toc}{section}{Conclusion}
\label{sec:conclusion_sprint0}

Le Sprint 0 a permis de poser les bases du projet \textit{Parc Informatique FMM} en définissant les rôles Scrum, en priorisant le backlog selon la méthode MoSCoW et en planifiant les sprints. Avec Hela Boussaid en charge du développement et Racha Zaibi en tant que Scrum Master, le projet peut entrer dans les phases de développement itératif. Les chapitres suivants détailleront l’implémentation des sprints et les résultats obtenus.


